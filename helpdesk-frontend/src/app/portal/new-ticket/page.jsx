    ...
    <div className="max-w-3xl mx-auto">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/portal/my-tickets">
          <Button variant="outline" size="sm">Cancel & Return</Button>
        </Link>
      </div>
      ...
      {submitted ? (
        <Card className="text-center p-8 bg-emerald-50/50 border-emerald-200 animate-slide-up">
          ...
          <p className="text-xs text-slate-600 mt-2 max-w-md mx-auto">
            Your ticket reference is <span className="font-mono font-bold text-slate-800">#TK-8942</span>.
          ...
          <CardContent className="p-6 md:p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <Input label="Office Location / Room Number" placeholder="e.g. Building A, Room 302" />
                <Input label="Contact Phone Number"          placeholder="+251 9..." />
                <Input label="Device Asset Tag (Optional)"   placeholder="e.g. INSA-LT-001" />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Select
                  ...
                  options={[
                    ...
                    { value: 'software', label: '⚙️ Software & Email' },
                    { value: 'access',   label: '🔑 SSO & Access Permission' },
                  ]}
                />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { id: 'LOW',    label: 'Low',           class: 'border-slate-200 text-slate-700' },
                      { id: 'MEDIUM', label: 'Medium',        class: 'border-brand-300 text-brand-700 bg-brand-50' },
                      { id: 'HIGH',   label: 'High / Critical', class: 'border-rose-300 text-rose-700 bg-rose-50' },
                    ].map((p) => (
                      <button
                        key={p.id}
                        type="button"
                        ...
                        className={`px-3 py-2 text-xs font-semibold rounded-xl border transition ${
                          priority === p.id ? p.class : 'border-slate-200 text-slate-500 hover:bg-slate-50'
                        }`}
                      >
                        {p.label}
                      </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="Specific Error Message (If Any)" placeholder="e.g. Error Code: 0x80070057" />
